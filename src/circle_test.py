...
def circle(t, T):
    x = jnp.array([0.000000, 0.707107, 0.000000, 0.707107, 0.6, 0.2 * jnp.sin(t / T * 2 * jnp.pi), 0.2 * jnp.cos(t / T * 2 * jnp.pi) + 0.8])
    return x

# sample poses on the path
path_nodes = septik.compute_points(circle, 10, 100)

# project points on each IK manifold
franka_urdf, franka, franka_coll = septik.load_robot("fr3_franka_hand.urdf")
X = septik.sample(septik.primes[0:7], jnp.arange(1, 21))
...
septik.jacobi_stein_ik(jnp.array(path_nodes[0]), 10, 25, X, franka)

print("(SEPTIK) Projecting")
layers = []
ts = time.perf_counter()
jsik_batch = jax.vmap(septik.jacobi_stein_ik, in_axes=(0, None, None, None, None))
layers = jsik_batch(jnp.array(path_nodes), 10, 25, X, franka)
# for i in range(len(path_nodes)):
#     X_proj = septik.jacobi_stein_ik(jnp.array(path_nodes[i]), 10, 25, X, franka)
...
waypts, path = septik.compute_path(layers, circle, franka, 10)
